...
class shopTextureHelper {
    ...
    public static function getTemplates($template_id = null, $read = true) {
        if ($template_id) {
            return self::getTemplate($template_id, $read);
        } else {
            $templates = array();
            foreach (shopTexturePlugin::$templates as $template_id => $template) {
                $templates[$template_id] = self::getTemplate($template_id, $read);
            }
            return $templates;
        }
    }
    ...
    protected static function getTemplate($template_id, $read = true) {
        if (empty(shopTexturePlugin::$templates[$template_id])) {
            return false;
        }

        $template = shopTexturePlugin::$templates[$template_id];
